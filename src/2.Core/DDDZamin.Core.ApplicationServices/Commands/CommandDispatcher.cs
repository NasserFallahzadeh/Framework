...
public class CommandDispatcher : ICommandDispatcher
{
    ...
    public async Task<CommandResult> Send<TCommand>(TCommand command) where TCommand : class, ICommand
    {
        ...
        {
            _logger.LogError(ex, "There is not suitable handler for {CommandType} Routing failed at {StartDateTime}.",
        ...
        finally
        {
            ...
            _logger.LogInformation(ZaminEventId.PerformanceMeasurement,
                "Processing the {CommandType} command tooks {Milliseconds} Milliseconds.", command.GetType(),
        ...
        where TCommand : class, ICommand<TData>
    {
        ...
        try
        {
            _logger.LogDebug("Routing command of type {CommandType} With value {Command} Start at {StartDateTime}",
        ...
        {
            _stopwatch.Stop();
            _logger.LogInformation("Processing the {CommandType} command tooks {Milliseconds} Milliseconds",
