...
public class CommandDispatcherDomainExceptionHandlerDecorator : CommandDispatcherDecorator
{
    ...
    public override async Task<CommandResult> Send<TCommand>(TCommand command)
    {
        ...
        {
            _logger.LogError(ZaminEventId.DomainValidationException, ex,
        ...
        catch (AggregateException ex)
        {
            ...
            {
                _logger.LogError(ZaminEventId.DomainValidationException, domainStateException,
                    "Processing of {CommandType} With value {Command} failed at {StartDateTime} because there are domain exception.",
                    command.GetType(), command, DateTime.Now);
    ...
    private string GetExceptionText(DomainStateException domainStateException)
    {
        ...
        _logger.LogInformation(ZaminEventId.DomainValidationException,
