    ...
    public static string ToQueryString(this object obj)
    {
        ...
        foreach (var key in propertyNames)
        {
            ...
            if (!valueElemType.IsPrimitive &&
                ...
                valueElemType != typeof(Guid)) 
                continue;

            var enumerable=properties.FirstOrDefault(c=>c.Key==key).Value as IEnumerable;

            foreach (var item in enumerable) properties.Add(KeyValuePair.Create(key, item));
        }

        return string.Join("&", properties.Where(x => x.Value is not null)
            .Select(x => string.Concat(Uri.EscapeDataString(x.Key), "=", Uri.EscapeDataString(x.Value.ToString()))));
    }
}